...
const Grid = struct {
    ...
    pub fn reset(self: *Self) void {
        // clear seen array
        for (self.seen) |*set| {
            set.setIntersection(set.complement());
        }
    }
    ...
    pub fn followBeam(self: *Self, x: usize, y: usize, dir: Dir) void {
        // check for cycle
        ...
        for (0..newDirCnt) |i| {
            ...
            if (self.move(&newX, &newY, newDirs[i])) {
                self.followBeam(newX, newY, newDirs[i]);
            }
        }
    }
...
pub fn run(input: []const u8) !void {
    ...
    defer grid.deinit();
    var maxCoverage: usize = 0;
    // top edge, going down
    for (0..grid.w) |x| {
        grid.followBeam(x, 0, .S);
        const coverage = grid.countSeen();
