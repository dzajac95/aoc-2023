...
const Grid = struct {
    ...
    pub fn countDiffV(self: Self, col: usize) usize {
    ...
    pub fn countDiffH(self: Self, row: usize) usize {
        ...
        while (rowIdxStack.popOrNull()) |i| {
            ...
            for (self.getRow(i), self.getRow(rIdx)) |top, bot| {
                if (top != bot) {
                    diff += 1;
                }
            }
...
pub fn run(input: []const u8) !void {
    ...
    while (chunks.next()) |chunk| cont: {
        ...
        for (1..grid.w) |col| {
            if (grid.countDiffV(col) == 1) {
                print("Vertical reflection with smudge at col {d}\n", .{col});
                sum += col;
                break :cont;
            }
        }
        for (1..grid.h) |row| {
            if (grid.countDiffH(row) == 1) {
                print("Horizontal reflection with smudge at row {d}\n", .{row});
                sum += row * 100;
                break :cont;
            }
        }
